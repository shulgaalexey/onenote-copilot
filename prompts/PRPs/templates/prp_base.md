...
### Current Codebase tree (run `tree /f` in PowerShell or `Get-ChildItem -Recurse` in the root of the project) to get an overview of the codebase
```powershell
...
# Test the endpoint (using Invoke-RestMethod for PowerShell or curl if available)
Invoke-RestMethod -Uri "http://localhost:8000/feature" -Method POST -ContentType "application/json" -Body '{"param": "test_value"}'

# Alternative with curl (if installed):
# curl -X POST http://localhost:8000/feature -H "Content-Type: application/json" -d '{\"param\": \"test_value\"}'
