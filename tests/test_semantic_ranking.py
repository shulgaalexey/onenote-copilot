...
from src.search.semantic_ranking import (
    SemanticRanking,
    QueryAnalysis,
    RankingWeights,
    RankingScore,
...
class TestRankingScore:
    ...
    def test_score_breakdown(self):
        ...
        assert "query_match" in breakdown  # Note: different key format
...
class TestSemanticRanking:
    ...
    def test_weights_updating(self, ranking_system):
        ...
        # Weights get normalized, so check they're proportional
        assert ranking_system.weights.query_match > ranking_system.weights.title_match
    ...
    def test_custom_weights_creation(self, ranking_system):
        ...
        custom_weights = ranking_system.create_custom_weights(
            emphasis="freshness"  # Correct parameter name
        )
        
        assert isinstance(custom_weights, RankingWeights)
        # Freshness emphasis should have higher freshness weight
        assert custom_weights.freshness > custom_weights.query_match
